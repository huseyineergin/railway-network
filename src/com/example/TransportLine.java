...
public enum TransportLine {
    ...
    private static final List<TransportLine> codeList = Arrays.asList(TransportLine.values());

    public String getLineName() {
        return lineName;
    }

    public static TransportLine findLine(String lineCode) {
        int idx = -1;
        for (TransportLine t : codeList) {
            if (t.toString().equals(lineCode)) {
                idx = codeList.indexOf(t);
            }
        }
        ...
        return codeList.get(idx);
    }

    TransportLine(String lineName) {
